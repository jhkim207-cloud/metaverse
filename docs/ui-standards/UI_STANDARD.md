...
### 1.6 다크/라이트 모드 필수 설계 규칙

> **모든 UI 컴포넌트 및 화면 설계 시 다크 모드와 라이트 모드를 반드시 함께 고려하여 설계한다.**

- 신규 컴포넌트/화면 설계 시 Light Mode와 Dark Mode **양쪽 모두에서** 시각적 검증 필수
- 색상은 반드시 CSS 변수(`var(--*)`)를 사용하고, `:root`(Light)와 `[data-theme="dark"]`(Dark) 양쪽에 정의
- 하드코딩 색상(`#hex`, `rgb()`, `rgba()`)을 직접 사용하면 다크/라이트 전환 시 깨지므로 금지
- 새로운 CSS 변수 추가 시 Light/Dark 값을 **동시에** 정의해야 함
- Glass 효과, 그림자, 투명도 등은 모드별로 가시성과 대비를 별도 확인
- **PRD UI 설계(Gate 2)** 시에도 다크/라이트 모드 양쪽의 컴포넌트 스타일을 명시할 것
...
### 3.1 3-Column 레이아웃 구조

헤더 + 좌측 네비게이션 + 센터 콘텐츠 + 우측 패널 구조를 기본으로 한다.

| 영역 | 역할 | 초기 너비 | 리사이즈 범위 |
|------|------|----------|-------------|
| **헤더** | 로고, 검색, 테마 전환, 사용자 정보 | 56px (고정 높이) | - |
| **좌측 패널** | 메뉴 네비게이션 | 220px | 180px ~ 400px |
| **센터 패널** | 메인 업무처리 화면 | flex (잔여 공간) | 최소 100px |
| **우측 패널** | 분석/보조 업무 화면 | 582px | 200px ~ 800px |

### 3.2 패널별 역할 정의

#### 센터 패널 (Main Work Area)
- **역할**: 사용자가 주요 업무를 수행하는 **메인 업무처리 화면**
- **용도**: 데이터 조회, 입력, 편집, 목록 관리 등 핵심 업무 기능
- **구성**:
  - **필터 헤더**: 상단 고정 영역 (업무 제목, 조회 조건, 필터, 검색/조회 버튼)
  - **콘텐츠 영역**: 스크롤 가능한 메인 작업 영역 (그리드, 폼, 카드 등)
- **설계 원칙**: 사용자의 주된 시선과 인터랙션이 집중되는 영역으로, 가장 넓은 공간 확보

#### 우측 패널 (Assist Panel)
- **역할**: 센터 패널의 업무 수행을 지원하는 **분석/보조 업무 화면**
- **용도**: 센터 패널에서 선택한 항목의 상세 정보, 관련 분석 데이터, 참고 자료, 부가 기능 제공
- **구성**:
  - **필터 헤더**: 상단 고정 영역 (패널 제목, 보조 필터)
  - **콘텐츠 영역**: 스크롤 가능한 보조 작업 영역
- **연동 패턴**:
  - 센터 패널에서 행/항목 선택 시 우측 패널에 상세 정보 표시
  - 센터 패널의 데이터에 대한 분석, 통계, 차트 등 보조 정보 제공
  - 센터 패널 업무 수행 시 필요한 참조 데이터, 이력, 메모 등 표시
- **설계 원칙**: 센터 패널의 업무 흐름을 방해하지 않으면서 필요한 정보를 즉시 제공

### 3.3 일반 규칙

- 모바일에서는 좌측 네비게이션을 드로어로 전환
- 모바일에서는 우측 패널을 숨기거나 하단 시트로 전환
...
- [ ] 공통 컴포넌트 재사용
- [ ] aria-* 속성 적용
- [ ] 다크/라이트 모드 양쪽에서 UI 검증 완료 (색상, 대비, Glass 효과, 그림자 등)
- [ ] 반응형 테스트
...
> 제조 워크플로우(수주→출고) 중심 업무 처리를 위한 네비게이션 아키텍처.
> 글로벌 베스트 프랙티스(MS Dynamics 365 BPF, Salesforce Path, Jira, Odoo)를 참조하여 설계.

### 17.1 개요: Hub-and-Spoke 모델

본 시스템은 **Hub-and-Spoke(허브 중심)** 네비게이션 모델을 따른다.

- **워크플로우 허브** (Main > 전체 Workflow): 전체 파이프라인을 조망하고, 각 단계의 업무에 진입하는 **중심 화면**
- **도메인 화면** (별도 메뉴): 각 단계별 상세 CRUD 업무를 처리하는 **전문 화면** (예: 수주관리, 생산관리, 출고관리)
- 사용자는 허브에서 도메인으로 **같은 탭 내 점프**, 작업 완료 후 **컨텍스트를 유지하며 복귀**

```
                     ┌─ 수주관리 (도메인) ─┐
                     │                     │
워크플로우 허브 ──── 점프 ──→ 업무 처리 ──→ 리턴 (컨텍스트 유지)
  (전체 파이프라인)   │                     │
                     ├─ 생산관리 (도메인) ─┤
                     ├─ 출고관리 (도메인) ─┤
                     └─ ...               ─┘
...
| 시스템 | 채택 패턴 | 적용 영역 |
|--------|----------|----------|
| MS Dynamics 365 BPF | 항상 보이는 단계 바 (Persistent Header) | 네비게이션 아키텍처 |
| Salesforce Lightning Path | 쉐브론 파이프라인 시각화 | 워크플로우 시각화 |
| Jira / GitHub Projects | 보드 + 우측 상세 패널 | 레이아웃 |
| Odoo / ERPNext | 브레드크럼 + 문서 체인 네비게이션 | 점프 & 리턴 |

### 17.2 Persistent Workflow Header (항상 보이는 단계 바)

> **규칙**: 워크플로우 관련 화면에서는 PipelineBar(스테퍼)가 **항상 상단에 표시**된다.

- 도메인 화면으로 점프해도 스테퍼는 **언마운트되지 않음** → 사용자는 현재 위치를 항상 인식
- MS Dynamics 365의 Business Process Flow 바와 동일한 개념
- 스테퍼의 각 단계 노드는 시각화이자 **네비게이션 컨트롤**을 겸함

**구현 방법: React Router 중첩 라우트**

부모 레이아웃에 스테퍼를 고정하고, `<Outlet>`으로 하위 화면을 렌더링한다.

```
라우트 구조:
/workflow                        → 허브 (칸반/그리드 뷰)
/workflow/:stageCode             → 단계별 필터 뷰
/workflow/:stageCode/:itemId     → 도메인 상세 화면 (점프 대상)
```

```
부모 레이아웃 (항상 마운트):
┌────────────────────────────────────────┐
│ PipelineBar (persistent, 언마운트 안됨)  │
├────────────────────────────────────────┤
│ <Outlet />  ← 하위 라우트 렌더링        │
│   · /workflow → KanbanBoard / Grid     │
│   · /workflow/:stageCode/:itemId       │
│     → 도메인 화면 (수주상세 등)          │
└────────────────────────────────────────┘
```
...
- 워크플로우 허브에서 항목 클릭 시 `navigate()`로 도메인 화면으로 이동 (같은 탭)
- 이동 시 `location.state`에 **리턴 컨텍스트**를 전달

```typescript
// 점프 시 리턴 컨텍스트 전달 예시
navigate(`/workflow/${stageCode}/${itemId}`, {
  state: {
    returnTo: '/workflow',
...
- **방법 1**: 브레드크럼 클릭 → `navigate(location.state.returnTo)`
- **방법 2**: 브라우저 Back 버튼
- 복귀 시 `location.state`의 값으로 **이전 상태를 복원** (스크롤 위치, 활성 단계, 뷰 모드)

#### 브레드크럼

형식: `Workflow Hub > [단계명] > [항목명]`

```
예시: Workflow Hub > 수주 > ORD-2026-001
      ↑ 클릭 가능    ↑ 클릭 가능  ↑ 현재 위치 (링크 아님)
```
...
| 금지 | 이유 |
|------|------|
| 도메인 화면을 모달/드로어로 열기 | AG Grid 등 전체 기능이 필요하므로 공간 부족 |
| 새 브라우저 탭으로 열기 | TanStack Query 캐시가 탭별 분리되어 데이터 동기화 불가 |
| 점프 시 워크플로우 허브를 언마운트 | 컨텍스트 손실 발생 |

### 17.4 컨텍스트 보존 규칙

> **규칙**: 점프 후 리턴 시 사용자가 떠나기 전 상태를 **그대로 복원**해야 한다.

| 보존 대상 | 메커니즘 | 설명 |
|----------|----------|------|
| 서버 데이터 (목록, 건수) | TanStack Query 캐시 | stale-while-revalidate로 캐시 즉시 표시 + 백그라운드 리프레시 |
| 활성 단계, 뷰 모드 | URL params | `/workflow?stage=PROD_ORDER&view=kanban` |
| 스크롤 위치, 선택 항목 | `location.state` | 점프 시 저장, 리턴 시 복원 |
| 필터/정렬 상태 | URL search params | `?sort=date&filter=HIGH` |

### 17.5 데이터 동기화 규칙

> **규칙**: 도메인 화면에서 데이터를 변경하면, 워크플로우 허브에 복귀 시 **즉시 반영**되어야 한다.

**메커니즘: TanStack Query Invalidation** (Section 9 패턴 확장)

도메인 화면에서 mutation 성공 시 워크플로우 관련 쿼리 키를 무효화한다.

```typescript
// Query Key 계층 구조
const workflowKeys = {
  all:        ['workflow'],
  stageCounts: () => [...workflowKeys.all, 'stage-counts'],
  stageItems:  (code: string) => [...workflowKeys.all, 'stages', code, 'items'],
  item:        (id: number) => [...workflowKeys.all, 'items', id],
};
```
...
| 액션 | 무효화 대상 | 전략 |
|------|-----------|------|
| 단계 이동 (예: 수주→주문) | `stageCounts`, `stageItems(이전)`, `stageItems(다음)` | Optimistic + invalidate |
| 항목 수정 | `item(id)`, `stageItems(현재)` | Optimistic + invalidate |
| 항목 생성 | `stageCounts`, `stageItems(대상)` | Invalidate on success |
| 도메인에서 복귀 | `stageCounts`, `stageItems(활성)` | stale-while-revalidate |

**staleTime 설정:**

| 쿼리 | staleTime | 근거 |
|------|-----------|------|
| 단계별 건수 (`stageCounts`) | 30초 | 빈번하지 않은 변경, 즉시 복귀 지원 |
| 단계 항목 목록 (`stageItems`) | 10초 | 비교적 빈번한 변경, 신선도 우선 |

복귀 시 캐시된 데이터를 즉시 표시(stale-while-revalidate)하고 백그라운드에서 최신 데이터를 가져오므로, 사용자는 **로딩 없이 즉시** 워크플로우를 확인할 수 있다.

### 17.6 워크플로우 레이아웃 배치

기존 3-Column 레이아웃(Section 3) 위에 구축한다.

#### 허브 화면 (Main > 전체 Workflow)
...
│ (220px)    │                                    │ (582px)          │
│            │ ┌──────────────────────────────┐   │                  │
│ [메뉴]     │ │ Filter Header (sticky)       │   │ [StageDetail     │
│            │ ├──────────────────────────────┤   │  Panel]          │
│            │ │ PipelineBar (스테퍼)          │   │                  │
│            │ │  [프로젝트]─┐                 │   │ 항목 선택 시     │
│            │ │  [임가공]──┘→[수주]→[주문]→...│   │ 상세 정보 표시   │
│            │ ├──────────────────────────────┤   │                  │
│            │ │ Toolbar: 단계명 + ViewSwitcher│   │ 미선택 시        │
│            │ ├──────────────────────────────┤   │ 안내 메시지      │
│            │ │ Content: Grid 또는 Kanban     │   │                  │
│            │ └──────────────────────────────┘   │                  │
└────────────┴────────────────────────────────────┴──────────────────┘
```
...
**핵심**: 도메인 화면 진입 시 센터 패널 콘텐츠만 교체되고, PipelineBar는 **항상 유지**.

### 17.7 워크플로우 컴포넌트 목록

> 스타일 상세는 각 컴포넌트의 소스 코드를 참조. 모든 컴포넌트는 CSS 변수(`var(--*)`) 기반으로 다크/라이트 모드 자동 호환.

| 컴포넌트 | 위치 | 용도 | 상태 |
|----------|------|------|------|
| `WorkflowStepper` | `components/workflow/` | 수평 파이프라인 바 (ENTRY/MAIN/BRANCH 3종 노드) | 구현 완료 |
| `KanbanBoard` | `components/workflow/` | 상태별 4컬럼 칸반 보드 (PENDING/IN_PROGRESS/COMPLETED/BLOCKED) | 구현 완료 |
| `KanbanCard` | `components/workflow/` | 칸반 보드 내 개별 항목 카드 (Glass 배경, hover 효과) | 구현 완료 |
| `StageDetailPanel` | `components/workflow/` | 우측 패널 항목 상세 (프로젝트 미니 스테퍼 포함) | 구현 완료 |
| `SegmentedControl` | `components/ui/` | 뷰 모드 전환 (그리드 \| 칸반) - 기존 공용 컴포넌트 재사용 | 구현 완료 |
| `WorkflowBreadcrumb` | - | 점프 & 리턴 경로 표시 (`Hub > 단계 > 항목`) | `[PROVISIONAL]` |
| `StageCard` | - | 단계별 KPI 미니 대시보드 (건수, 상태 분포, 평균 처리시간) | `[PROVISIONAL]` |

**워크플로우 정의 파일:** `frontend/src/constants/workflow.ts`
- 13개 단계 정의 (ENTRY 2 + MAIN 7 + BRANCH 4)
- 상태/우선순위 색상 설정 (`STATUS_CONFIG`, `PRIORITY_CONFIG`)

**워크플로우 훅:** `frontend/src/hooks/useWorkflow.ts`
- `useWorkflowCounts()`, `useStageItems()`, `useWorkflowItem()`
- 현재 useState/useEffect 기반 → Section 17.5의 TanStack Query 패턴으로 마이그레이션 권장
