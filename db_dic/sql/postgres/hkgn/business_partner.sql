-- ============================================
-- 거래처 마스터 테이블 (business_partner)
-- ============================================
-- 설명: 매출처/매입처/외주처 통합 관리
...
-- 기존 테이블 삭제
DROP TABLE IF EXISTS hkgn.business_partner CASCADE;
...
CREATE TABLE hkgn.business_partner (
    ...
    bp_cd             VARCHAR(30)   NOT NULL UNIQUE,                           -- [거래처코드] 거래처 식별 코드 (UNIQUE)
...
-- 인덱스
-- ============================================
CREATE INDEX idx_bp_cd ON hkgn.business_partner(bp_cd);
...
COMMENT ON COLUMN hkgn.business_partner.id IS '[식별자] 기본 키 (PK, 자동증가)';
COMMENT ON COLUMN hkgn.business_partner.bp_cd IS '[거래처코드] 거래처 식별 코드 (UNIQUE)';
